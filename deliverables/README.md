...
- **"Buddy Tony"** (`prg/tony-buddy.prg`, 37,945 B) — the Colonnade plus a
  second, GREEN Tony that the game controls: he follows the player, keeps a
  polite distance, always turns to face him, and hops whenever the player
  jumps. Same character model (he wears the player's own animation frames on
  the two spare enemy sprites), colored independently of the color scheme.
  Touching him is harmless — the enemy collision check is masked to the bats.
  Generator: `tools/make_buddy.py`; screenshots `screenshots/buddy-*.png`
  (`buddy-4-hop.png` catches both Tonys mid-air).

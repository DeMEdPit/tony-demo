...
| `tony-chamber-the-shadow.prg` | 40,248 | the same build with the token's behaviour and a provisional colour stamped: The Shadow (Follow, green): keeps his distance, faces Tony, jumps when Tony jumps (Tony's own arc, measured) and crouches when he crouches. `tools/verify_buddy.py` is the scripted test of all seven |
...
| `tony-chamber-the-shy-one.prg` | 40,248 | the same build with the token's behaviour and a provisional colour stamped: The Shy One (Shy, light red): runs when Tony comes close, cowers at the pillar, creeps back when he leaves. `tools/verify_buddy.py` is the scripted test of all seven |

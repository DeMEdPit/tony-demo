...
| `tony-chamber-the-shadow.prg` | 40,248 | the same build with the token's behaviour and a provisional colour stamped: The Shadow (Follow, green): keeps his distance, faces Tony, jumps when Tony jumps (Tony's own arc, measured). `tools/verify_buddy.py` is the scripted test of all seven |
...
| `tony-chamber-the-mirror.prg` | 40,248 | the same build with the token's behaviour and a provisional colour stamped: The Mirror (Mirror, light blue): Tony's reflection about the room's centre line, jumps with him. `tools/verify_buddy.py` is the scripted test of all seven |

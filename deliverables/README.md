...
| `tony-chamber-the-mirror.prg` | 40,248 | the same build with the token's behaviour and a provisional colour stamped: The Mirror (Mirror, light blue): Tony's reflection about the room's centre line; crouches while Tony is in the air, bounces while Tony is crouched. `tools/verify_buddy.py` is the scripted test of all seven |
